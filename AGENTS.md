...
5. 行为变更按 MECE 分支分析和 `docs/engineering/TDD_WORKFLOW.md` 先让子 agent 写或审测试。
...
跨层变更必须在 OpenSpec design 中记录这些控制点，并用 MECE 思维拆分关键分支：
...
- 测试设计必须覆盖 MECE 分支分析中的关键状态、失败、权限、边界和副作用。
