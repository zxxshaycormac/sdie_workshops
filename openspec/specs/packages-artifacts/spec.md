...
Baseline specification of Gitea's package registry, container registry, release management, LFS, attachments, and storage backend subsystems. All requirements describe the current (v1.22.x) system behavior.
...
## 7. Storage Backends (PKG-07)

**User Story:** As a system administrator, I want to configure different storage backends for different artifact types so that I can optimize cost and performance per use case.
...
- **PKG-07-101:** `When the system stores an artifact, the system shall write the content to the configured backend for that artifact type.`
- **PKG-07-102:** `When the system retrieves an artifact, the system shall read the content from the configured backend for that artifact type.`
- **PKG-07-103:** `When a storage backend fails during a write operation, the system shall return an error and not persist partial data.`
...
- **PKG-07-301:** `If a configured storage backend is unreachable at startup, then the system shall log an error and fail to start the affected subsystem.`
- **PKG-07-302:** `If a direct URL generation request fails (expired credentials, missing bucket), then the system shall fall back to proxied serving through the Gitea server.`
- **PKG-07-303:** `If an artifact storage path is not writable, then the system shall reject write operations with a storage error.`
