...
**Frequency**: universal; every non-trivial template calls at least one helper. New helpers that touch `models/*` go in `modules/templates/util_*.go` and follow the structural-concession cluster documented in `modules/CLAUDE.md` Section 1.
...
**Why**: Mirroring the route makes it trivial to find the template from a URL and vice versa; the `tplXxx base.TplName` constant in the handler (see `routers/web/CLAUDE.md` Section 4) is the single source of truth for the path string. `.tmpl` is the only extension — 486 of 486 files use it.
...
**Why**: Inline scripts break webpack bundling, escape the CSRF/fetch wrapper, and break CSP-friendly deployment. The `tw-` prefix lets Tailwind utilities override Fomantic UI without specificity wars. These rules are owned by `web_src/CLAUDE.md` Sections 3 and 5 — they apply equally to templates and are restated here only as a pointer.
...
**Exceptions**: `templates/base/head_script.tmpl` must stay inline because it bootstraps `window.config` before the JS bundle loads. New exceptions require a TODO and a structural justification (e.g. avoiding FOUC). See `web_src/CLAUDE.md` Section 3 for the full rationale.
